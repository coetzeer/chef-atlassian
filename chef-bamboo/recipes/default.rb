...
execute "link data directory" do
  ...
  COMMAND
  creates "#{node['bpl']['devtool']['install_home']}/data"
end
...
execute "setting up #{node['bpl']['devtool']['name']}" do
  ...
  command <<-COMMAND
    ...
    tar -xvf atlassian-bamboo-5.4.1.tar.gz
    ln -s atlassian-bamboo-5.4.1 #{node['bpl']['devtool']['name']}
  COMMAND
end
...
template "#{node['bpl']['devtool']['install_dir']}/atlassian-bamboo/WEB-INF/classes/bamboo-init.properties" do
    source "bamboo_init_properties.erb"
    mode 00700
    owner node['bpl']['devtool']['user']
    group node['bpl']['devtool']['user']
end
